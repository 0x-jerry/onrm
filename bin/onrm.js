...
program.command("ls").description("List all the registries").action(onLs);
...
program.command("help").description("Print this help").action(onHelp);
...
async function onLs() {
  ...
  registries.forEach((r, i) => {
    msg +=
      i === currentRegIndex.npm || i === currentRegIndex.yarn ? "* " : "  ";

    msg += (r.name + " ").padEnd(10, "-") + " ";
    msg += r.registry + " ";

    let pkg =
      (i === currentRegIndex.npm ? "npm " : " ") +
      (i === currentRegIndex.yarn ? "yarn" : "");
...
async function onUse(name, type) {
  ...
  const msg = info.find
    ? `set ( ${pkg.trim().split(/\s/).join(", ")} ) registry to ${
        info.find.registry
      }`
...
async function getRegistriesWithCurrent() {
  ...
  if (info.yarn) {
    currentRegIndex.yarn = regs.findIndex((r) =>
      info.yarn.includes(r.registry)
    );
  }
...
async function setCurrentRegistry(name, type) {
  ...
  if (find) {
    msg = await execPkgManagerCommand(
      `config set registry ${find.registry}`,
      type
    );
  }
